...
class PostgreSQLConnection:
    ...
    def select_user(self, query):
        if not self.conn:
            print("Conexão ainda não foi estabelecida")
            return None
        try:
    ...
    def insert_user(self, id, name, area, job_description, role, salary, is_active, last_evaluation):
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(f"INSERT INTO users VALUES ({id}, '{name}', '{area}', '{job_description}', {role}, {salary}, {is_active}, '{last_evaluation}')")
            self.conn.commit()
            cursor.close()
            print("Usário inserido com sucesso!")
        except psycopg2.Error as e:
    ...
    def delete_user(self, user_id):
        ...
        try:
            ...
            self.conn.commit()
            cursor.close()
            print("Usário excluído com sucesso!")
        except psycopg2.Error as e:
            print("Não foi possível excluir o usuário:", e)
